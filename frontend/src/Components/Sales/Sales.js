  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // ✅ Merge here directly without storing products state
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-3 items-center">
          ...
          <button
            onClick={() =>
              window.open("http://localhost:5000/api/sales/report/pdf", "_blank")
            }
          ...
          <tbody>
            {sales && sales.length > 0 ? (
              sales
                ...
                .map((sale) => (
                  <SalesD key={sale.SalesId} sale={sale} onDelete={handleDelete} />
                ))
            ) : (
              <tr>
