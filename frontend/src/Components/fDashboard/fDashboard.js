...
function ProfitLossDashboard() {
  const [month, setMonth] = useState(""); // Filter for cards
  const [year, setYear] = useState("");
  const [result, setResult] = useState(null);
  const [expenses, setExpenses] = useState([]);
  const [loading, setLoading] = useState(false);
  const [pieMonth, setPieMonth] = useState("all"); // For pie chart filter
  ...
  const filteredExpenses = pieMonth === "all"
    ? expenses
  ...
  return (
    <div className="min-h-screen flex bg-gray-100">
      ...
      <div className="ml-52 flex-1 p-6 flex flex-col items-center gap-6">
              ...
              >
                <div className="flex flex-col">
                  ...
                  <span className="text-gray-500 mt-0.5 text-xs">
                    {month && year ? `${months[month - 1]} ${year}` : "All Time"}
                  </span>
                </div>
              ...
              {recentExpenses.length === 0 ? (
                ...
                <ul className="space-y-2 flex-1 overflow-y-auto pr-1">
                  {recentExpenses.map((exp, idx) => (
                    <li key={idx} className="p-3 rounded-md border border-gray-100 bg-gray-50 hover:bg-gray-100 shadow-sm transition flex justify-between items-center">
                      <div className="flex flex-col">
                        <span className="inline-block px-2 py-0.5 text-[10px] font-bold rounded-full w-fit mb-1 bg-gray-200 text-gray-800">
                          {exp.category || "Other"}
                        </span>
                        <p className="text-gray-800 font-medium text-sm">{exp.description || "No description"}</p>
                      ...
                      <div className="text-right">
                        <p className="text-sm font-bold text-red-600">- Rs {exp.amount.toFixed(2)}</p>
                      </div>
                    </li>
                  ))}
                </ul>
